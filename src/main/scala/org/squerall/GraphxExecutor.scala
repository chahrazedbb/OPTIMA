  ...
  :Graph[String, String] = {
    ...
    while ({it.hasNext}) {
      ...
      if (firstTime) {
        ...
        println("this is gph1")
        f1.collect.foreach(println(_))
        println("this is gph2")
        gph2.edges.collect.foreach(println(_))
        ...
        val s: RDD[String] = jGrah.triplets.map(triplet =>triplet.srcAttr + " " + triplet.attr + " " + triplet.dstAttr)
        println("this is sssssssssss")
        s.collect.foreach(println(_))
      } else {
